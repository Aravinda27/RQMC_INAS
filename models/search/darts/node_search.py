    ...
    def __init__(self, node_steps, node_multiplier,device,args):
        ...
        for i in range(self.node_steps):
            for j in range(self.num_input_nodes+i):
                edge_op = FusionMixedOp(self.C, self.L, self.args,self.device)
...
class FusionNode(nn.Module):
    ...
    def __init__(self, node_steps, node_multiplier,device, args):
        ...
        self.node_steps = node_steps
        self.node_multiplier = node_multiplier
        self.node_cell = NodeCell(node_steps, node_multiplier,device, args)
    ...
    def forward(self, x, y):
        ...
        #edge_weights = gumbel_rao(self.betas, k = 10, temp=0.1)
        #node_weights = gumbel_rao(self.gammas, k = 10, temp = 0.1)
...
if __name__ == '__main__':
    ...
    args = Args(16, 8)
    node_cell = NodeCell(2, 1,device, args)
    fusion_node = FusionNode(2, 1,device, args)
