...
def plot(genotype, filename, args, task=None):
    ...
    input_nodes_A = [f"Frame_{i+1}" for i in range(num_input_nodes)]
    input_nodes = input_nodes_A  # RGB-only
    assert len(input_nodes) == num_input_nodes, f"Expected {num_input_nodes} input nodes, got {len(input_nodes)}"
    ...
    for i in range(steps):
        ...
        with g.subgraph(name=step_node_name, node_attr={'shape': 'box'}) as c:
            ...
            for j in range(node_multiplier):
