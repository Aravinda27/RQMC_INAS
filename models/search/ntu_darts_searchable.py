...
class Searchable_Skeleton_Image_Net(nn.Module):
    def __init__(self, args, criterion, logger,device):
        ...
        self.rgbnet = ntu.Visual(args).to(device)
        self.skenet = ntu.Spectrum(args).to(device)
    ...
    def forward(self, tensor_tuple):
        ...
        print("shape of visual features",len(visual_features))
        print("Shape of sketeal features",len(skel_features))
    
        
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

        
        input_features = self.reshape_input_features(visual_features)
        print("Shape after reshaping",len(visual_features))
